...
export function fromJSON(obj: types.LiquidityCalculationModeJSON): types.LiquidityCalculationModeKind {
  switch (obj.kind) {
    case 'Deposit': {
      return new Deposit();
    }
...
export function layout(property?: string) {
  const ret = borsh.rustEnum([borsh.struct([], 'Deposit'), borsh.struct([], 'Withdraw')]);
