...
export function fromJSON(obj: types.ExecutiveWithdrawActionJSON): types.ExecutiveWithdrawActionKind {
  switch (obj.kind) {
    case 'Freeze': {
      return new Freeze();
    }
...
export function layout(property?: string) {
  const ret = borsh.rustEnum([borsh.struct([], 'Freeze'), borsh.struct([], 'Unfreeze'), borsh.struct([], 'Rebalance')]);
