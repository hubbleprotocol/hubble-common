...
export class ScopeChainAccount {
  ...
  static readonly discriminator = Buffer.from([180, 51, 138, 247, 240, 173, 119, 79]);

  static readonly layout = borsh.struct([borsh.array(borsh.array(borsh.u16(), 4), 512, 'chainArray')]);

  constructor(fields: ScopeChainAccountFields) {
    this.chainArray = fields.chainArray;
  }

  static async fetch(c: Connection, address: PublicKey): Promise<ScopeChainAccount | null> {
  ...
  static async fetchMultiple(c: Connection, addresses: PublicKey[]): Promise<Array<ScopeChainAccount | null>> {
