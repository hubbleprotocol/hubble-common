...
export class GlobalConfig {
  ...
  static readonly discriminator = Buffer.from([149, 8, 156, 202, 160, 252, 176, 217]);
  ...
  static async fetch(c: Connection, address: PublicKey): Promise<GlobalConfig | null> {
  ...
  static async fetchMultiple(c: Connection, addresses: PublicKey[]): Promise<Array<GlobalConfig | null>> {
  ...
  static fromJSON(obj: GlobalConfigJSON): GlobalConfig {
    return new GlobalConfig({
      ...
      swapRewardsDiscountBps: obj.swapRewardsDiscountBps.map((item) => new BN(item)),
      actionsAuthority: new PublicKey(obj.actionsAuthority),
      adminAuthority: new PublicKey(obj.adminAuthority),
      treasuryFeeVaults: obj.treasuryFeeVaults.map((item) => new PublicKey(item)),
