...
export class Kamino {
  ...
  getWhirlpools = async (whirlpools: PublicKey[]): Promise<(Whirlpool | null)[]> => {
    ...
    const fetchedMap: Record<string, Whirlpool | null> = fetched
      .reduce((map, whirlpool, i) => {
  ...
  getRaydiumPools = async (pools: PublicKey[]): Promise<(PoolState | null)[]> => {
    const poolStrings = pools.map((whirlpool) => whirlpool.toBase58());
    ...
    const fetchedMap: Record<string, PoolState | null> = fetched
      .reduce((map, whirlpool, i) => {
