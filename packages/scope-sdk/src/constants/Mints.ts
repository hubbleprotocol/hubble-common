...
export const ScopeMints: { token: SupportedToken; mint: string }[] = [
...
export function scopeTokenToMint(token: SupportedToken) {
  return ScopeMints.find((x) => x.token === token)?.mint;
}

export function mintToScopeToken(mint: string) {
  return ScopeMints.find((x) => x.mint === mint)?.token;
}
